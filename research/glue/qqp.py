...
def write_tfrecord(
    ...
    def get_tfrecord_example(data):
        ...
        for f in data:
            input_ids_s1 = (
                [tokenizer.cls_token] + tokenizer.tokenize(f['question1'])[: max_seq_length - 2] + [tokenizer.sep_token]
            ...
            input_ids_s2 = tokenizer.tokenize(f['question2'])[: max_seq_length - 1] + [
                tokenizer.sep_token
            ]  # -1 to add SEP
...
def run_qqp(cfg: DictConfig):
    logging.info("Run QQP")
    cfg_task = compose(config_name="config", overrides=["+glue=qqp"])
