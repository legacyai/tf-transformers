        ...
        if model_checkpoint_dir:
            ...
            if convert_from_hf and not load_succesfuly:
                ...
                if convert_fn_type == "tf":
                    cls_ref.convert_hf_to_tf(model, convert_tf_fn=convert_gpt2_tf, convert_pt_fn=None)
                if convert_fn_type == "pt":
                    cls_ref.convert_hf_to_tf(model, convert_tf_fn=None, convert_pt_fn=convert_gpt2_pt)
        if return_layer:
            return model_layer, config
        return model, config
