...
def convert_t5_pt(model, config):
    """PT converter
    Args:
    ...
    def convert(model_name):
        ...
        model_hf = T5Model.from_pretrained(model_name)
        ...
        inputs_tf = {}
        inputs_tf["encoder_input_ids"] = tf.cast(tf.constant(inputs["input_ids"].numpy()), tf.int32)
        inputs_tf["encoder_input_mask"] = tf.ones_like(inputs_tf["encoder_input_ids"])
        inputs_tf["decoder_input_ids"] = tf.cast(tf.constant(inputs["input_ids"].numpy()), tf.int32)
...
def convert_t5_tf(model, config):
    """TF converter
    Args:
        model_hf: HuggingFace Model (TF)
        model: tf_transformers model/layer
    ...
    def convert(model_name):
        ...
        # Final Layer Norm weight
        mapping_dict[
            "tf_t5model/encoder/final_layer_norm/weight:0"
        ] = "tf_transformers/t5_encoder/last_layer_norm/weight:0"
        ...
        # Final Layer Norm weight
        mapping_dict[
            "tf_t5model/decoder/final_layer_norm/weight:0"
        ] = "tf_transformers/t5_decoder/last_layer_norm/weight:0"
        ...
        assert np.allclose(outputs_hf, outputs_tf, rtol=0.5) == True
