...
# ==============================================================================
"""TF 2.0 CLIP Image Encoder Model"""
...
from tf_transformers.layers.transformer import TransformerCLIP
...
@add_start_docstrings(
    "CLIP Image Model :",
    ENCODER_CLASS_DOCSTRING.format("tf_transformers.models.clip.CLIPImageConfig"),
)
class CLIPImageEncoder(LegacyLayer):
    def __init__(
        self,
        config: Dict,
        ...
        **kwargs,
        ...
        self._embedding_layer = PatchEmbeddings(
            config['image_size'], config['patch_size'], config['num_channels'], config['embedding_size'], use_bias=False
        )
        self._positional_embedding_layer = PositionEmbeddingImage(self._num_patches, config['embedding_size'])

        # # Pre Embedding Norm (applies before passing to transformer)
    ...
    def call_encoder(self, inputs: Dict[str, Union[tf.keras.layers.Input, tf.Tensor]]) -> Dict[str, tf.Tensor]:
        ...
        token_embeddings = self._last_layer_norm(token_embeddings)
    ...
    def call_encoder_auto_regressive(self, inputs):
        raise NotImplementedError("CLIP as of now not supports decoding")

    def call_decoder(self, inputs):
        raise NotImplementedError("CLIP as of now not supports Decoder")

    def call_decoder_auto_regressive(self, inputs):
        raise NotImplementedError("CLIP as of now not supports Seq2Seq decoding")

    @add_start_docstrings(
        "CLIP Image Call method :",
        MAIN_CALL_DOCSTRING,
    )
