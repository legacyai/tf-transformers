...
class AlbertEncoder(LegacyLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            config: dict
            mask_mode: str, `user_defined` BERT by default uses
            masking for PADDED or MLM. But can be overridden . # noqa
            name: str, Name of the model
            use_dropout: bool, It is strictly optional. Sometimes,
                         while training you can set `use_dropout` to False.
                         If `is_training` is False, `use_dropout`
                         will be automatically set to False. # noqa
            ...
            bidirectional: use in relative postional embedding
            (we can infer it based on mask_mode)
            is_decoder: bool, if True it will become decoder mode (as in Seq2Seq)
            use_mlm_layer: bool ( To use MLM layer or not )
            share_encoder_embeddings: bool, When is_decoder = True,
                            most cases, it will re-use the embedding layer from Encoder.
            ...
            return_all_layer_token_embeddings: If to return all layer token embeddings
        """
        ...
        self.embedding_projection_size = config["embedding_projection_size"]
        ...
        super(AlbertEncoder, self).__init__(is_training=self.is_training, use_dropout=self.use_dropout, **kwargs)
        self._config_dict = {
            ...
            "return_all_layer_token_embeddings": return_all_layer_token_embeddings,
        }
        ...
        self._embedding_layer, self._type_embeddings, self._position_embedding_layer = self.get_embedding_layers(
            self.embedding_projection_size
        )
        if self.is_decoder:
        ...
        # Albert Intermediate Projection
        self._intermediate_embedding_projection = dense_einsum.DenseEinsum(
            output_shape=self.embedding_size,
            activation=None,
            kernel_initializer=tf.keras.initializers.get(initializer),
            bias_initializer=tf.keras.initializers.get("zeros"),
        ...
        for i in range(self.num_hidden_layers):
            self._transformer_layers.append(self.layer)
        ...
        self._lower_embedding_projection = tf.keras.layers.Dense(
            units=self.embedding_projection_size,
            activation=self.activation,
            kernel_initializer=self.initializer,
            name="lower_embedding_projection",
        )
    ...
    def call_predict(self, inputs):
        ...
        # Step 0 of inference. For step0, we do not have valid cache. \
        # We pass zero tensor
        ...
        #  the given inputs is the first input # noqa
        ...
        # MLM Projection
        if self.use_mlm_layer:
            # This matrix won't be used after pretraining
            token_embeddings = self._lower_embedding_projection(token_embeddings)
        ...
        past_length, last_token_tensor = tf.cond(
            tf.equal(tf.reduce_sum(past_length), 0),
            lambda: step_0_gather(past_length, token_embeddings_projected),
            lambda: step_other_gather(past_length, token_embeddings_projected),
        )
        ...
        return {
            "token_embeddings": token_embeddings_projected,
    ...
    def call_training(self, inputs):
        ...
        Args:
            inputs: dict
            inputs is a dict with keys  [`input_ids` , \
                `input_mask`, `input_type_ids`].
            These keys might or might not be present \
                based on `mask_mode` and other criterias
        ...
        if self.use_mlm_layer:
            ...
            token_logits = (
                tf.matmul(
                    tf.cast(token_embeddings, tf.float32),
                    self.get_embedding_table(),
                    transpose_b=True,
                    name="token_logits",
                )
    ...
    def call_decoder_predict(self, inputs):
        """Inputs will be pass to this method, when \
            is_training = False and is_decoder = True.
        The need to cache the past `key` and `value`
    ...
    def call(self, inputs):
        ...
        Testing pipeline has few changes to accomodate caching of
        `key` and `value` for Transformer.
    ...
    def show_sample_model_inputs_outputs(self):
        """Show the sample inputs , so that users won't get
        confused with various settings"""
    ...
    def extend_positional_embeddings(self, factor):
        ...
        self._config_dict["max_position_embeddings"] = 2 * self._config_dict["max_position_embeddings"]
