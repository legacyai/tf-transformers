...
def map_to_dict(item):
    """Convert text to a dict"""
    return {"text": item}

def mlm_fn():
    max_seq_len = 1024
    max_predictions_per_batch = 155
    delimiter = '__||__'
    ...
    def dynamic_mlm(example):
        ...
        if slice_prob <= 0.05:
            ...
            if tf.greater(max_segment_shape, MIN_WORDS):
                random_slice_index = tf.random.uniform(minval=MIN_WORDS, 
                                                    maxval=max_segment_shape,
                                                    shape=(),
                                                    dtype=tf.int32)
...
masked_lm_map = mlm_fn()
# Train Dataset

ds_train = tf.data.TextLineDataset(tf.io.gfile.glob("{}/*.txt".format(DATA_DIR)))

train_dataset = ds_train.map(map_to_dict, 
                            num_parallel_calls=tf.data.AUTOTUNE
                            )
train_dataset = train_dataset.map(masked_lm_map, num_parallel_calls=tf.data.AUTOTUNE)
