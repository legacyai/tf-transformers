...
def get_distribution_strategy(
    ...
    if not isinstance(distribution_strategy, str):
        ...
        if distribution_strategy is False:  # pylint: disable=singleton-comparison,g-explicit-bool-comparison
            msg += (
...
def configure_cluster(worker_hosts=None, task_index=-1):
    ...
    if tf_config:
        ...
        os.environ["TF_CONFIG"] = json.dumps(
            {"cluster": {"worker": workers}, "task": {"type": "worker", "index": task_index}}
        )
    else:
        num_workers = 1
    return num_workers
