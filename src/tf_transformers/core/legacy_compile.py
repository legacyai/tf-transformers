...
class LossesContainer(Container):
    ...
    def metrics(self):
        ...
        if not self._built:
            return []
        per_output_metrics = [
            metric_obj for metric_obj in nest.flatten(self._per_output_metrics) if metric_obj is not None
        ]
    ...
    def build(self, y_pred):
        ...
        self.loss_items = {}
        for key, loss_function in self._user_losses.items():
            if loss_function is None:
                continue
            self.loss_items[key] = self._get_loss_object(loss_function)
        self._create_metrics()
        if self._loss_weights is None:
            self._loss_weights = {}
            for key in self.loss_items:
                self._loss_weights[key] = None
    ...
    def _create_metrics(self):
        ...
        self._per_output_metrics = {}
        for key in self.loss_items:
            self._per_output_metrics[key] = metrics_mod.Mean("{}_loss".format(key))
    ...
    def __call__(self, y_true, y_pred, sample_weight=None, regularization_losses=None):
        ...
        #       loss_value = loss_obj(y_true_original, y_pred_original, sample_weight=sw)
        #       print("loss obj", loss_obj, loss_value)
        ...
        batch_dim = None
        loss_weight = None
        for key, loss_obj in self.loss_items.items():

            loss_value = loss_obj(y_true_original, y_pred_original)
            loss_metric_value = loss_value
            # Correct for the `Mean` loss metrics counting each replica as a batch.
            if loss_obj.reduction == losses_utils.ReductionV2.SUM:
                loss_metric_value *= ds_context.get_strategy().num_replicas_in_sync
            ...
            if batch_dim is None:
                batch_dim = array_ops.shape(y_pred_original[key])[0]
            metric_obj = self._per_output_metrics[key]
            metric_obj.update_state(loss_metric_value, sample_weight=batch_dim)

            loss_weight = self._loss_weights[key]
            if loss_weight is not None:
                loss_value *= loss_weight
                loss_metric_value *= loss_weight
...
class MetricsContainer(Container):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        try:
            ...
            for y_t, y_p, sw, metric_objs, weighted_metric_objs in zip(*zip_args):
                ...
                if y_t is None or (
                    all(m is None for m in metric_objs) and all(wm is None for wm in weighted_metric_objs)
                ):
                    continue
    ...
    def _should_broadcast(self, obj):
        ...
        if not nest.is_sequence(obj):
            return True
        # e.g. ['mse'] or ['mse', 'mae'].
        return isinstance(obj, (list, tuple)) and not any(nest.is_sequence(o) for o in obj)
...
def map_to_output_names(y_pred, output_names, struct):
    ...
    outputs_are_flat_list = (
        not single_output and isinstance(y_pred, (list, tuple)) and not any(nest.is_sequence(y_p) for y_p in y_pred)
    )
    ...
    if (single_output or outputs_are_flat_list) and isinstance(struct, dict):
        ...
        if struct:
            raise ValueError(
                ...
                "to any Model output: {}. Expected: {}".format(struct.keys(), output_names)
            )
        if len(new_struct) == 1:
            return new_struct[0]
        return new_struct
